...
def get_current_datetime(time: str) -> datetime:
    dt = datetime.now()
    hour, minute = get_time_components(time)
...
def get_current_time_arg() -> str:
    arg = str(sys.argv[1])
    valid_arg = validate_current_time_re(arg)
    if valid_arg:
        return arg
    else:
...
def validate_current_time_re(time: str) -> bool:
    valid_re = bool(re.match(CURRENT_TIME_RE, time))
    return valid_re


def get_time_components(time: str) -> Time:
